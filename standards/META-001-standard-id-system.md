...
#### 4.18.4 Exempt list (44 files, 18 579 lines, as of 2026-06-19)
...
**Externalised references (37 files, 14 036 lines):**
...
Promote file-size limits matrix to META-001 as canonical source.

- Added §4.18 "File Size Limits (Canonical Source)" with 6 subsections:
  §4.18.1 Limits matrix (12 categories, hard limit + soft warn)
  §4.18.2 How to pick a category (path > filename > frontmatter id prefix)
  §4.18.3 Parser-bound files (own ceiling per category, not blanket 250)
  §4.18.4 Exempt list (44 files, 18 579 lines — full table inline)
  §4.18.5 Domain standard references (STD-SKILL-001 §8.2, STD-FE-001 §6,
         STD-DOC-002 §11 should reference, not duplicate)
  §4.18.6 Change history for §4.18
- Rationale: previously the limits matrix lived only inside
  RULE-MONOLITH-012 v1.2 (L2 enforcement layer). This violated
  layering — L2 was defining WHAT (limits) instead of just HOW
  (enforcement). §4.18 makes L1 the canonical source; RULE-MONOLITH-012
  v1.3 retains a compact mirror marked "see META-001 §4.18 for
  canonical" but defers to this section.
- Coordinated change: RULE-MONOLITH-012 v1.2 -> v1.3 (Z-ai-guard
  commit pending), STD-SKILL-001 §8.2 / §10.1 / §13 updated to
  reference §4.18 instead of restating the 800-line ceiling.
- No new IDs. No graph changes. 13/13 HARD PASS maintained.
